...
public class FormulaBienFormada {
	...
	/**
	 * Metodo que permite obtener las clasusulas en un {@link ArrayList} de
	 * {@link ArrayList} de {@link String}
	 * 
	 * @author Cristian Giovanny Sanchez Pineda
	 * @param fbf formula bien formada
	 * @return un {@link ArrayList}
	 */
	public ArrayList<ArrayList<String>> getClausules(String fbf) {
		FormulaBienFormada fbf1 = new FormulaBienFormada(fbf);
		ArrayList<String> fnc = fbf1.toFC();
		ArrayList<ArrayList<String>> clausulas = new ArrayList<ArrayList<String>>();
		ArrayList<String> newClausula;
		String clausula = "";
		for (int i = 0; i < fnc.size(); i++) {
			clausula = fnc.get(i);
			newClausula = new ArrayList<String>();
			for (int j = 0; j < clausula.length(); j++) {
				if (clausula.charAt(j) == '¬') {
					newClausula.add(clausula.charAt(j) + "" + clausula.charAt(j + 1));
					j = j + 1;
				} else {
					newClausula.add(clausula.charAt(j) + "");
				}
			}
			clausulas.add(newClausula);
		}
		return clausulas;
	}
